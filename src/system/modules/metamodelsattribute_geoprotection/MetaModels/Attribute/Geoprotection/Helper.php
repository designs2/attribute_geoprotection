...
class Helper
{
		...
		if (empty(self::$arrCountries))
		{
			...
			foreach ($countriesByContinent as $strConKey => $arrCountries)
			{
				...
				foreach (array_keys($arrCountries) as $key)
				{
					$arrTmp[$key] = array
					(
						'name'         => strlen($GLOBALS['TL_LANG']['CNT'][$key]) ? utf8_romanize($GLOBALS['TL_LANG']['CNT'][$key]) : $countries[$key],
	...
	public static function getCountriesByContinent($arrValues = array())
	{
		...
		{
			$arrReturn[$GLOBALS['TL_LANG']['CONTINENT']['other']]['xx'] = strlen($GLOBALS['TL_LANG']['CNT']['xx']) ? $GLOBALS['TL_LANG']['CNT']['xx'] : 'No Country';
		}
